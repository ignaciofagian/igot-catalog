...
export default function HomePage() {
  const [abbreviatures, setAbbreviatures] = useState<Abbreviature[] | null>(
    null
  );
  const [attributes, setAttributes] = useState<Attribute[] | null>(null);

  const serverAttributes = () => {
    return getAttributeList().then((attrs: Attribute[]) =>
      setAttributes(attrs)
    );
  };

  const serverAbbreviatures = () => {
    return getAbbreviatureList().then((abbs: Abbreviature[]) =>
      setAbbreviatures(abbs)
    );
  };
